...
- 📸 **区域截图与标注**：全局热键 `Alt+P` 唤起，框选区域或单击自动捕获窗口，支持画笔、矩形、椭圆、箭头、文字、撤销和重选
- 📌 **无边框钉图**：截图可直接临时钉在桌面最前方，无需永久保存；拖动移动、右键关闭
- 🪟 **透明置顶窗口**：不挡视线，随时可用
...
- **截图与标注**：任意窗口下按 `Alt+P` 或右键菜单进入。WinPet 会先隐藏宠物自身，再以物理像素抓取 Windows 虚拟桌面作为底图（支持高 DPI 缩放与多显示器）；拖动框选区域，或单击自动捕获光标下的窗口；进入标注模式后可使用画笔、矩形、椭圆、箭头和文字，支持撤销与重新框选。
- **保存**：只有主动点击“保存”时，才会把已标注截图永久写入系统“图片/WinPet”目录，并立即返回完整文件路径。
- **钉住**：与保存完全独立。点击“钉住”只会把已标注截图写入 WinPet 临时目录并创建独立的无边框、始终置顶窗口，不会写入“图片/WinPet”。关闭钉图时对应临时文件会自动删除。
- **钉图交互**：按住钉图可拖动位置，右键即可关闭；钉图窗口不占任务栏，可同时存在多个。
...
flowchart TD
  ...
  A --> G[Screen Capture / PNG Save / Temporary Pin]
  G --> H[React Canvas Screenshot Editor]
```

- **后端**（Rust / Tauri）：负责系统采样、窗口与托盘，以及 EXE/CMD、纯文本剪贴板、屏幕底图抓取、PNG 持久保存和临时钉图窗口
...
| `src/ScreenshotEditor.jsx` | 区域框选、画笔/规则图形/文字标注、保存、临时钉图视图 |
| `src/ScreenshotEditor.css` | 截图编辑器与无边框钉图样式 |
| `src-tauri/src/lib.rs` | 系统监控、托盘/窗口事件、EXE/CMD、剪贴板、截图落盘与临时钉图窗口 |
...
| 层级 | 技术 |
|------|------|
| 后端 | Tauri 2.11 / Rust / sysinfo / tauri-plugin-global-shortcut / Windows PowerShell |
